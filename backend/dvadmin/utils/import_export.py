...
@Created on: 2022/2/25 025 22:29
@Remark:
"""
...
def export_excel(field_data: list, data: list, FileName: str, file_path: str = settings.MEDIA_ROOT):
    ...
    for i in range(len(col_width)):
        if col_width[i] > 10:
            width = col_width[i] if col_width[i] < 36 else 40
    ...
    for results in data:
        for index, values in enumerate(results.values()):
            ...
            sheet.row(index).height =  20*40  #20为基准数，40意为40磅
...
def export_excel_save_model(request, field_data, data, FilName):
    ...
    savefile, _ = FileList.objects.get_or_create(url=file_rul)
    if _ == True:
...
def import_to_data(file_url, field_data):
    ...
    """
    # 读取excel 文件
    file_path = os.path.join(settings.MEDIA_ROOT, file_url.replace('/','\\'))
    ...
    data = xlrd.open_workbook(file_path_dir)
    table = data.sheets()[0]
    # 创建一个空列表，存储Excel的数据
    tables = []
